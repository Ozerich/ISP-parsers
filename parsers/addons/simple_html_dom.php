...
class simple_html_dom_node {
    ...
    function dump($show_attr=true) {
        self::dump_html_tree($this, $show_attr);
    }
	...
	static function dump_html_tree($node, $show_attr=true, $deep=0) {
	    ...
	    foreach($node->nodes as $c)
	        self::dump_html_tree($c, $show_attr, $deep+1);
	}
...
class simple_html_dom {
    ...
    // clean up memory due to php5 circular references memory leak...
    function clear() {
        foreach($this->nodes as $n) {if($n) $n->clear(); $n = null;}
        if (isset($this->parent)) {if($this->parent) $this->parent->clear(); unset($this->parent);}
        if (isset($this->root)) {if($this->root) $this->root->clear(); unset($this->root);}
        unset($this->doc);
        unset($this->noise);
    }
